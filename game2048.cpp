...
struct Game2048 {
    ...
    // Calculate and print the score based on the current board state
    void score() {
        int score = 0;
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
    ...
    // Check if the game has ended (no empty spaces)
    bool end() {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                if (board[i][j] == 0) {
    ...
    // Push and merge tiles in a single row or column
    void push(int array[]) {
        int last_added = 4;
        int merge = 1;
        for (int i = 3; i >= 0; i--) {
